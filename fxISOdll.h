...
#define FxISO_IncompleteDllSet          21
...
// Acquisition error specification (added to FxISO_Acquisition Error)
	...
	#define FxISO_SCAN_BusyInCapture 19
...
// PARAMETERS
//         fxISO: Output - a string representing FxISO SDK version is copied into a user provided char array
//                      The array must be allocated by the caller (at least 15 bytes).
//                      The string has up to 15 ASCII characters, null terminated.
//                      If the pointer is NULL, the string is not returned
//     fxISOscan: Output - a string representing FxISOscan.dll version is copied into a user provided char array
//                      The array must be allocated by the caller (at least 15 bytes).
//                      The string has up to 15 ASCII characters, null terminated.
//                      If the pointer is NULL, the string is not returned
//   fxISOenrdlg: Output - a string representing fxISOenrdlg.dll version is copied into a user provided char array
//                      The array must be allocated by the caller (at least 15 bytes).
//                      The string has up to 15 ASCII characters, null terminated.
//                      If the pointer is NULL, the string is not returned
...
//------------------------------------------------------------------------------
// ---------- Functions to check NFIQ and PAD on acquired fingerprint ---------
//------------------------------------------------------------------------------
// Functions that works on acquired fingerprint and returns:
//	+ standard NIST Fingerprint Image Quality (nfiq) score, 
//  + Presentation Attack Detection (PAD) score
//------------------------------------------------------------------------------

_fxexport FXISOerror _fxcall FxISO_Fing_GetNFIQ(int* quality100, int* majorVersion);
// Compute NFIQ score on acquired fingerprint
// PARAMETERS
//   quality100:    Output - quality score from 0 (bad) to 100 (good).
//                             if NFIQ is version 1, there are 5 discrete levels: {0,10,20,30,50}
//   majorVersion:  Output - NFIQ major version (2 if NFIQ-2 dll are found)
//                             this parameter is optional and NULL can be passed
// 
// RETURNED VALUE
//   FxISO_OK: the function succeeded
//   FxISO_IncompleteDllSet: FxISOImageTools.dll not found

_fxexport FXISOerror _fxcall FxISO_Fing_GetPAD(int* padScore100);
// Compute Presentation Attach Detection (PAD) score on acquired fingerprint
// PARAMETERS
//   padScore100: Output - PAD score from 0 (real finger) to 100 (fake finger).
// 
// RETURNED VALUE
//   FxISO_OK: the function succeeded
...
typedef struct
{
	...
	BYTE resolution;		//image DPI as ResolutionMode
	BYTE reserved1;
